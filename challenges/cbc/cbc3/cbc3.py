...
def main():
    global secret, encrypted_secret

    secret = gen_flag()
    key = urandom(32)
    id = base64.b64encode(urandom(16)).decode('utf-8')

    encrypted_secret = {'id': id, 'encrypted_password': base64.b64encode(encrypt(secret.encode('utf-8'), key)).decode('utf-8')}
...
def check_password():
    ...
    key = users[id]
    if secret.encode('utf-8') == decrypt(encrypted_pass, key):
